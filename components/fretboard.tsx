...
import { noteToSolfege, getNoteFromFret, getScaleNotes } from "@/lib/music-utils"
...
export function Fretboard({ rootNote, scaleType, notationType }: FretboardProps) {
    ...
    <div className="relative min-w-[800px]">
      ...
      <div className="absolute top-0 left-0 right-0 flex pointer-events-none">
        ...
        {Array.from({ length: FRETS + 1 }).map((_, fret) => {
          ...
          return (
            <div key={fret} className="flex-1 flex items-start justify-center pt-1">
              {showMarker && (
                <div className="flex flex-col gap-1">
                  <div className="w-2 h-2 rounded-full bg-muted-foreground/20" />
                  {isDoubleMarker && <div className="w-2 h-2 rounded-full bg-muted-foreground/20" />}
                </div>
              )}
            </div>
          )
      ...
      <div className="space-y-3 pt-8">
        {STRINGS.map((openString, stringIndex) => (
          <div key={stringIndex} className="flex items-center gap-2">
            {/* String label */}
            <div className="w-10 text-right text-xs font-medium text-muted-foreground">{openString}</div>
              ...
              {Array.from({ length: FRETS + 1 }).map((_, fret) => {
                ...
                return (
                  <div key={fret} className="flex-1 flex items-center justify-center relative">
                    {/* Fret line */}
                    {fret > 0 && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-px h-8 bg-border" />}
      ...
      <div className="flex mt-4">
        ...
        {Array.from({ length: FRETS + 1 }).map((_, fret) => (
          <div key={fret} className="flex-1 flex items-center justify-center">
            <span className="text-xs text-muted-foreground font-mono">{fret}</span>
          </div>
        ))}
      </div>
    </div>
